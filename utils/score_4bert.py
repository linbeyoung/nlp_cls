...
def show():
    ...
    scores = {
    'Precision' : [result[genre]['precision'] for genre in [0,1,2,3,'all']],
    'Recall' : [result[genre]['recall'] for genre in [0,1,2,3,'all']],
    'F1' : [result[genre]['f1'] for genre in [0,1,2,3,'all']],
    }
...
def save_to_file(output_filename):
    global df
    df.to_csv(output_filename, sep='\t',index=True, header=True,encoding='utf-8',float_format='%.2f')
